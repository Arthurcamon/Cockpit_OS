...
│
├── services/
│   ├── deezer\\\_api.py    # Accès aux données Deezer (API publique)
│   ├── deezer\\\_player.py # Lancer une piste dans Deezer Desktop
│   ├── windows\\\_media.py # Contrôle Windows Media Session (play/pause/next…)
│   └── windows\\\_audio.py # Contrôle audio Windows (volume, périphériques)
│
└── web/
...
|`deezer.playlist.tracks`|`playlist\\\_id`|Morceaux d'une playlist|
|`deezer.album.tracks`|`album\\\_id`|Morceaux d'un album|
|`deezer.artist.albums`|`artist\\\_id`|Albums d'un artiste|
|`deezer.play`|`track\\\_id`, `title`|Lance la piste dans Deezer Desktop|
